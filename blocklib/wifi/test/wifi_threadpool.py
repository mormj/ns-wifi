...
def main():
    ...
    if 1: # through sync_short
        ...
        # fg.connect(sync_long, 0, fft_blk, 0).set_custom_buffer(gr.cuda_buffer_properties.make(gr.cuda_buffer_type.D2H).set_buffer_size(buf_size))
        fg.connect(fft_blk, 0, packetize, 0).set_custom_buffer(gr.cuda_buffer_properties.make(gr.cuda_buffer_type.D2H).set_buffer_size(buf_size))
        # fg.connect(fft_blk, 0, packetize, 0).set_custom_buffer(gr.vmcirc_buffer_properties.make(gr.vmcirc_buffer_type.AUTO).set_buffer_size(buf_size))
